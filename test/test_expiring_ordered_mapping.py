    ...
    def test_simple(self):
        with TemporaryDirectory() as temporary_directory:
            ...
            with Database(db_path) as db, db:
                ...
                self.assertEqual(tuple(d.keys(ExpiringOrderedMapping.Order.KEY)), ('baz', 'foo'))
                self.assertEqual(
                    tuple(d.items(ExpiringOrderedMapping.Order.KEY)),
                    (('baz', 1337), ('foo', 'bar')),
                )
                self.assertEqual(tuple(d.values(ExpiringOrderedMapping.Order.KEY)), (1337, 'bar'))

                self.assertEqual(tuple(reversed(d.keys(ExpiringOrderedMapping.Order.KEY))), ('foo', 'baz'))
                self.assertEqual(
                    tuple(reversed(d.items(ExpiringOrderedMapping.Order.KEY))),
                    (('foo', 'bar'), ('baz', 1337)),
                )
                self.assertEqual(tuple(reversed(d.values(ExpiringOrderedMapping.Order.KEY))), ('bar', 1337))
